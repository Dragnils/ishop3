...
@WebServlet("/ajax/html/more/products")// обработчик ajax
public class AllProductsMoreController extends AbstractController { // обрабатывает поведение нажатия кнопки "Load more products" в конце страницв
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        List<Product> products = getProductService().listAllProducts(getPage(req) , Constants.MAX_PRODUCTS_PER_HTML_PAGE); //get Products from database, мы должны получить из БД список всех продуктов с помощью класса Product
